...
public abstract class AbstractClientController extends AbstractController {
    ...
    @Resource
    protected ProxyInfoRPCService proxyRpcService;
    ...
    protected ProxyUserDto getUserDto() {
        if (getRequest().getSession().getAttribute("userDto") != null) {
            return (ProxyUserDto) getRequest().getSession().getAttribute("userDto");
        }
        ...
        if (StringUtils.isBlank(token)) {
            ...
            for (Cookie item : request.getCookies()) {
                if (StringUtils.equals(item.getName(), "pToken")) {
                    tokenCookie = item;
                    break;
                }
            }
        ...
        RPCResult<ProxyUserDto> result = proxyRpcService.verificationToken(token);
