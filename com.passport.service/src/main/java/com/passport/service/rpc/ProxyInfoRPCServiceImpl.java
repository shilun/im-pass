...
public class ProxyInfoRPCServiceImpl extends StatusRpcServiceImpl implements ProxyInfoRPCService {
    ...
    @Override
    public RPCResult<Boolean> addUser(String proxyId, String pin, String phone, String pass, String desc, Long resources[]) {
        RPCResult<Boolean> result = new RPCResult<>();
        try {
            ...
            log.error("ProxyInfoRPCService.addUser proxyId={},phone={},pass={},desc={},roles={}", new String[]{proxyId, phone, pass, desc, Arrays.toString(resources)}, e);
            result.setCode("system.error");
            result.setMessage("未知错误");
        }
        return result;
    ...
    @Override
    public RPCResult<Boolean> upUser(String proxyId, String pin, String phone, String desc,  boolean status,Long[] resources) {
        RPCResult<Boolean> result = new RPCResult<>();
        try {
            proxyUserInfoService.upUser(proxyId, pin, phone, desc, status, resources);
            result.setSuccess(true);
        } catch (BizException e) {
            result.setException(e);
        } catch (Exception e) {
            log.error("ProxyInfoRPCService.upUser proxyId={},pin={},phone={},desc={},roles={},status={}", new String[]{proxyId, pin, phone, desc, Arrays.toString(resources), Boolean.toString(status)}, e);
            result.setCode("system.error");
            result.setMessage("未知错误");
        }
        return result;
